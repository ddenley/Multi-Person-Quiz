    ...
    def print_responses_testing(self, api_responses):
        ...
        for response in api_responses:
            ...
            if not result.is_final:
                ...
                print(dialog)
                # Add dialogue to queue logic
                current_speaker = None
                sentence = ""
                length_of_dialog = len(dialog)
                i = 0
                for word in dialog:
                    i += 1
                    if current_speaker == None:
                        current_speaker = word[1]
                    if current_speaker == word[1]:
                        sentence = sentence + " " + word[0]
                    # Detected a new user has started to speak
                    # Place in queue and start new sentence
                    else:
                        self.transcription_queue.put("{}: {}".format(word[1], word[0]))
                        sentence = ""
                        current_speaker = word[1]
                    # Ensure last sentence is also added to queue
                    if i == length_of_dialog and current_speaker != word[1]:
                        print("Placed last sentence")
                        self.transcription_queue.put("{}: {}".format(word[1], word[0]))
