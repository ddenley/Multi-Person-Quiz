...
def stt_get() -> Tuple[str, int, int]:
    """Requests a diarized conversation turn from the stt module."""
...
def gui_post(img_path: str) -> int:
    """POST to gui server. Returns the response code as an int."""
    response = requests.post(url=f"http://{__hostname}:{__gui_port}/update_flag", json={'filePath': img_path})
    return response.status_code
